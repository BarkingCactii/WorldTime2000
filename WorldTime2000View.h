...
// Operations
private:
     void DrawGradient (CMemDC& pDC, CRect* pRect);
     TFXDataTip m_datatip;
    bool FormatToolTip ( CPoint point );

public:
	void OnDoTimer( void );
	void ClearDocPtr ( void ) { m_pDocument = 0; }    

     void DrawTooltip ( CMemDC & memDC );
     void DrawHome ( CMemDC &, CTime * );
     void DrawLocation ( CMemDC & memDC, CTime * );
	 void DrawTime ( CMemDC & memDC, CTime * t, int y, float offset, int );
