...
//	li->alarmEnable = m_enable.GetCheck ();
	if ( enable == GDT_VALID )
	{
		...
		a->alarmEnable = 1;
		a->alarmHour = time.GetHour();
          if ( a->alarmHour >= 12 )
               a->alarmHour -= 12;

		a->alarmMinute = time.GetMinute();
...
//          m_ampm.SetCurSel ( 0 );
	...
	a->alarmMessage = buf;
     ...
     a->alarmRunApp = appToRun;
     a->alarmRunEnable = m_runApp.GetCheck ();
...
//	m_enable.Create( DTS_SHOWNONE, rect, this, 1000 ); //, 1000 );
	...
	if ( view->selected >= 0 )
     ...
     appToRun = a->alarmRunApp;

	SetDlgItemText ( IDC_LOCATION, li->city );
	SetDlgItemText ( IDC_APP_NAME, appToRun );
	SetDlgItemText ( IDC_EDIT1, a->alarmMessage );
     m_runApp.SetCheck ( a->alarmRunEnable );
	...
	CTime time ( 1999, 1, 1, a->alarmHour, a->alarmMinute, 0 );
	...
	DateTime_SetSystemtime ( m_enable, a->alarmEnable ? GDT_VALID : GDT_NONE, &st );
