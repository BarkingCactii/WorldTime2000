...
void Name ( string & output, DisplayItem * mItem, LocationItem * lItem )
{
    if ( mItem->Location() )
        output += lItem->city;
    else
...
void EMail ( string & output, DisplayItem * mItem )
{
    output += mItem->Email();
}

void Phone ( string & output, DisplayItem * mItem )
{
    output += mItem->Phone();
}
...
LocationItem * GetLocData ( int whichLoc, DisplayItem* &mItem )
{
    /*
...
void COnLineTimeView::OnContextModify() 
{
    ...
    DisplayItem * mItem = m.GetMemberByIdx ( whichLoc );
    if ( mItem == 0 )
        return;
...
void COnLineTimeView::OnToolsEmail() 
{
    ...
    for ( int i = 0; i < displayData.numDisplayLists; i++ )
    {
        if ( PtInRect ( displayData.data[i].bounds, contextMenuLoc ))
        {
            DisplayItem * mItem = m.GetMemberByIdx ( i );
            if ( mItem )
            {
