...
short TFXDataTip::m_s_delay = 500;
...
void TFXDataTip::Display( )
{
	...
	pos = m_listStrings.GetHeadPosition();
	while( pos )
	{
	...
	CRect wndRect(
			m_TipPosition.x,
			m_TipPosition.y,
			m_TipPosition.x + sizeAll.cx,
			m_TipPosition.y + sizeAll.cy);
...
void TFXDataTip::OnPaint() 
{
	...
	{
		str = m_listStrings.GetNext(pos);
		dc.TextOut((rect.Width( ) / 2), (_border / 2)+cy, str);
