...
bool ProcessChimes ( CTime & localTime )
{
    ...
    if ( localTime.GetMinute() == 0 )
    {
        ...
        {
            chimeSounded = true;    
            Playsound ( r.GetKey ("SoundHourFile").str.c_str(), r.GetKey ( "PlayHour").word ? true : false ); //r.SoundAlarm() ? true : false);
        }
    }
    else
    if ((localTime.GetMinute() % 15) == 0 )
    {
        ...
        {
            chimeSounded = true;
            Playsound ( r.GetKey ("SoundQuarterFile").str.c_str(), r.GetKey("PlayQuarter").word ? true : false ); //r.SoundAlarm() ? true : false);
        }
    }
    else
        chimeSounded = false;
...
//    LocationItem * lItem = 0;
    ...
    for ( int i = 0; dItem = m.GetMemberByIdx ( i ); i++ )
    {
        LocationItem * lItem = l.GetLocationbyTag ( dItem->LocRef() );
        if ( !lItem )
            continue;
        ...
        for ( int j = 0; aItem = dItem->GetAlarm ( j ); j++ )
        {
...
//                    alarmSet = mItem->alarmActive = true;
                    ...
                    // play a sound
                    Playsound ( r.GetKey ("SoundAlarmFile").str.c_str(), aItem->PlaySound() ? ( r.GetKey("PlayAlarm").word ? true : false) : false ); //r.SoundAlarm() ? true : false);
                    ...
                    strstream sout;
                    if ( !dItem->Location())
                        sout << dItem->Name().c_str() << " @ ";
                    sout << lItem->city.c_str() << ", " << lItem->country.c_str() << ends;
