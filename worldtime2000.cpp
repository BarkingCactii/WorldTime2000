...
RegistryCollection r ("software\\WorldTime2000");
Locations l;
Members m;
...
BOOL CWorldTime2000App::InitInstance()
{
    ...
    // Always perform these checks to make sure everything is ok:

//    InitData();

    if (!m_dlgSkin.Create(IDD_SKINDIALOG_DIALOG) || !m_dlgSkin.m_hWnd || !m_pSkin)
    {
     ...
     whatIfIcon = LoadIcon ( IDI_WHATIF );
     
	 timeOfDay[0] = LoadIcon ( IDI_TIME12 );
 	 timeOfDay[1] = LoadIcon ( IDI_TIME13 );
 	 timeOfDay[2] = LoadIcon ( IDI_TIME14 );
	 timeOfDay[3] = LoadIcon ( IDI_TIME1 );
	 timeOfDay[4] = LoadIcon ( IDI_TIME2 );
 	 timeOfDay[5] = LoadIcon ( IDI_TIME3 );
 	 timeOfDay[6] = LoadIcon ( IDI_TIME4 );
	 timeOfDay[7] = LoadIcon ( IDI_TIME5 );
 	 timeOfDay[8] = LoadIcon ( IDI_TIME6 );
 	 timeOfDay[9] = LoadIcon ( IDI_TIME7 );
 	 timeOfDay[10] = LoadIcon ( IDI_TIME8 );
 	 timeOfDay[11] = LoadIcon ( IDI_TIME9 );
 	 timeOfDay[12] = LoadIcon ( IDI_TIME10 );
 	 timeOfDay[13] = LoadIcon ( IDI_TIME11 );
...
//     m_list.SetTextColor ( 0x0000ffff );
     ...
     m_list.InsertColumn(0, "Location", LVCFMT_LEFT, rect.Width() / 2 - 45, 0);
     m_list.InsertColumn(1, "Country", LVCFMT_LEFT, rect.Width() / 2 - 45, 1);
     ...
     for ( int i = 0; i < l.Size(); i++ )
     {
          ...
          lvitem.iItem = i;
          lvitem.iSubItem = 0;
		  string crap = l.CityList(i);
          lvitem.pszText = (char *)crap.c_str();
          lvitem.lParam = l.Tag ( i );
          
          
          if ( selectionMade == false )
          {          
               if ( selectedbyString == l.CityList ( i ).c_str())
               {
          ...
          lvitem.iItem = i;
          lvitem.iSubItem = 1;
		  string crap2 = l.CountryList(i);
          lvitem.pszText = (char *)crap2.c_str();
          lvitem.lParam = l.Tag ( i );
