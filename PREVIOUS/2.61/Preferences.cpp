...
void CPreferences::OnOK() 
{
	...
	l.p.seconds = m_seconds.GetCheck ();
	l.p.tooltips = m_tooltips.GetCheck ();
    ...
    l.p.alarmOn = ( Boolean )m_alarmOn.GetCheck ();
    l.p.hourOn = ( Boolean )m_hourOn.GetCheck ();
    l.p.quarterOn = ( Boolean )m_quarterOn.GetCheck ();
...
//	VERIFY(m_home.SubclassDlgItem (IDC_HOME_SAMPLE, this));
	...
	m_seconds.SetCheck ( l.p.seconds );
	m_tooltips.SetCheck ( l.p.tooltips );
	m_drawSphere.SetCheck ( l.p.drawSphere );
	m_imageOn.SetCheck ( l.p.showWallpaper );
     m_movingSphere.SetCheck ( l.p.movingSphere );
     m_showAM.SetCheck ( l.p.showAM );

     m_alarmOn.SetCheck ( l.p.alarmOn );
     m_hourOn.SetCheck ( l.p.hourOn );
     m_quarterOn.SetCheck ( l.p.quarterOn );
