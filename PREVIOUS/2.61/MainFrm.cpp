...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	*/
	...
	m_wndToolBar.SetButtonInfo(7, ID_HELP_CONTENTS, TBSTYLE_BUTTON | TBSTYLE_DROPDOWN, 7);
...
void CMainFrame::OnSortCountry() 
{
	...
	while ( sorted == False )
	{
		...
		for ( int i = 0; aItem2 = l.Get ( i + 1 ); i++ )
		{
			aItem1 = l.Get ( i );
			...
			li1 = l.GetLocationbyTag ( *aItem1 );
			li2 = l.GetLocationbyTag ( *aItem2 );
			
			if ( ascending )
			{
				if ( stricmp ( li2->country, li1->country ) < 0 )
				{
					short tmp = *aItem2;
					*aItem2 = *aItem1;
					*aItem1 = tmp;
...
void CMainFrame::OnSortName() 
{
	...
	while ( sorted == False )
	{
		...
		for ( int i = 0; aItem2 = l.Get ( i + 1 ); i++ )
		{
			...
			if ( ascending )
			{
				if ( stricmp ( li2->city, li1->city ) < 0 )
				{
					short tmp = *aItem2;
					*aItem2 = *aItem1;
					*aItem1 = tmp;
