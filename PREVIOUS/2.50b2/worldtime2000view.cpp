...
static int dstIdx = 0;
Boolean mouseInView = False;	// is mouse in this view ?
...
//          selectedAdjustment = l.OffsetList ( l.OffsetDisplay ( selected ));
     ...
     {
          short * lIdx = l.Get ( tooltipLocation ); //selected );
          if ( lIdx )
...
/*
     ...
     memDC->Pie ( &rect, CPoint ( rect.right, rect.top - 20 ), CPoint ( rect.left, rect.top - 20 )); 
//     memDC->Chord ( &rect, CPoint ( rect.left + 10, rect.bottom ), CPoint ( rect.left + 10, rect.top - 50 ));
     memDC->RoundRect ( &rrect, CPoint ( 10, 10 ) );
  */      
     memDC->SelectObject ( saveFont );
...
//          static flashWhatIf = 1;
          if ( l.showWhatIf && flashWhatIf )
          {
               DrawIconEx(  memDC, (int)(( XHour * ratio ) - 28), YPOS + 2, theApp.whatIfIcon, 12, 12, 0, NULL, DI_NORMAL );
//               flashWhatIf = 0;
          }
          else
          {
...
//                    sprintf ( buf, "%s (%s)", li->city, li->country );
                    ...
                    sprintf ( buf, "%s (%d)", li->country, li->countryCode );

                    memDC->TextOut ( X + size.cx + 5, YPOS, ( char * ) buf ); //li->city );
...
void CWorldTime2000View::OnDraw ( CDC * dc )
{
     ...
     if ( l.showHoursGMT )
          ratio = ( float ) ( l.frameRect.right - l.frameRect.left - 15 ) / ( XDiff + 60.0f );
     else
          ratio = ( float ) ( l.frameRect.right - l.frameRect.left - 15 ) / ( XDiff );
//static const float XDiff = 315;
...
//          float offset = _timezone - selectedAdjustment - homeDST;
          float minOffset = ceil ( offset ) - offset; 
          minOffset *= 60.0f;
